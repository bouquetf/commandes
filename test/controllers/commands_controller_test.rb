...
class CommandsControllerTest < ActionController::TestCase
  ...
  test "should create command" do
    assert_difference('Command.count') do
      post :create, command: { date: @command.date, description: @command.description, limit_date: @command.limit_date, payment: @command.payment, state: @command.state, total: @command.total }
    end
  ...
  test "should update command" do
    patch :update, id: @command, command: { date: @command.date, description: @command.description, limit_date: @command.limit_date, payment: @command.payment, state: @command.state, total: @command.total }
    assert_redirected_to command_path(assigns(:command))
  end
